...
import { cancelCallback } from '../actions/cancelCallback'
...
export function CancelCallbackModal({ isOpen, callback, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!callback || !reason.trim()) return
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="Why are you cancelling this callback?"
              rows={3}
              maxLength={500}
              required
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {loading ? 'Cancelling...' : 'Cancel Callback'}
            </button>
          </div>
        </form>
      </div>
